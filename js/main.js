...
const updateData = (data) => {
  ...
  sunriseHTML.textContent = `${sunriseGMT.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  })}`;
  sunsetHTML.textContent = `${sunsetGMT.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  })}`;
...
const fetchQuery = (query) => {
  ...
  fetch(
    `https://api.openweathermap.org/geo/1.0/direct?q=${query}&limit=5&appid=${OPENWEATHER_APIKEY}&units=metric`
  )
    .then((resp) => resp.json())
    .then((data) => {
      if (data.length > 0) {
        data.forEach((d) => {
          capturedOptions.push(d);
        });
...
const displayOptionsHTML = (data) => {
  if (data.length === 1) {
    ...
    results.forEach((result) => {
      ...
      option.setAttribute(
        "value",
        `${requestedLocation.name} ${requestedLocation.state} ${requestedLocation.country}`
      );
      datalist.appendChild(option);
    });
  }
